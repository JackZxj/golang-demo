...
import (
	...
	"github.com/JackZxj/golang-demo/tstoragedemo/internal/cgroup"
	"github.com/JackZxj/golang-demo/tstoragedemo/internal/timerpool"
)
...
// Cabin provides goroutine safe capabilities of insertion into and retrieval from the time-series storage.
...
type Reader interface {
	// Select gives back a list of data points that matches a set of the given name
	// within the given start-end range. Keep in mind that start is inclusive, end is exclusive,
	// and both must be Unix timestamp. ErrNoDataPoints will be returned if no data points found.
	Select(name string, start, end int64) (points []*DataPoint, err error)
}

// Row includes a data point along with name.
type Row struct {
	// The unique name.
...
// Option is an optional setting for NewCabin.
type Option func(*cabin)
...
// NewCabin gives back a new storage, which stores time-series data in the process memory by default.
//
...
func NewCabin(opts ...Option) (Cabin, error) {
	s := &cabin{
...
func (s *cabin) InsertRows(rows []Row) error {
	s.wg.Add(1)
	defer s.wg.Done()
...
func (s *cabin) Select(name string, start, end int64) ([]*DataPoint, error) {
	if name == "" {
	...
	for iterator.next() {
		...
		ps, err := part.selectDataPoints(name, start, end)
		if errors.Is(err, ErrNoDataPoints) {
			continue
		}
		if err != nil {
...
func (s *cabin) flushPartitions() error {
...
func (s *cabin) flush(dirPath string, m *memoryPartition) error {
	if dirPath == "" {
...
func (s *cabin) removeExpiredPartitions() error {
...
func (s *cabin) inMemoryMode() bool {
	return s.dataPath == ""
}
